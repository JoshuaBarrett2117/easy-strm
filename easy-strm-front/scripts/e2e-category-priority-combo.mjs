...
async function run() {
  ...
  try {
    ...
    const sourceRow = page.locator("tbody tr").filter({ hasText: `category_priority_${stamp}` }).first();
    ...
    const fileRow = browserDialog.locator("tbody tr").filter({ hasText: "Priority.Combo.Movie.Release.mkv" }).first();
    ...
    const row = organizeDialog.locator("tbody tr").filter({ hasText: "Priority.Combo.Movie.Release.mkv" }).first();
