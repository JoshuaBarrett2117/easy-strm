...
async function runTests() {
  ...
  try {
    ...
    // 测试新增按钮
    const addBtn = await page.locator('button:has-text(\"新增账号\")');
    ...
    // 测试编辑按钮
    const editBtn = await page.locator('button:has-text(\"编辑\")').first();
    ...
    // 测试扫码更新按钮
    const qrBtn = await page.locator('button:has-text(\"扫码更新\")').first();
    ...
    if (await deleteBtn.isVisible()) {
      ...
      await page.waitForTimeout(500);
      const confirmBtn = await page.locator('.el-button--danger:has-text(\"确定\")');
    ...
    // 测试新增配置按钮
    const addConfigBtn = await page.locator('button:has-text(\"新增配置\")');
    ...
    const taskBtn = await page.locator('button:has-text(\"查看任务\")');
    ...
    // === 测试 8: 查看日志按钮 ===
    const logBtn = await page.locator('button:has-text(\"查看日志\")');
    ...
    const logoutBtn = await page.locator('button:has-text(\"退出登录\")');
