  ...
  <div class="share-records-page">
    <n-card title="分享管理">
      <n-space class="mb-4 share-toolbar">
        ...
        <n-button type="success" :disabled="!selectedShareIDs.length" @click="batch(false)">批量自动识别</n-button>
        <n-button type="success" secondary :disabled="!selectedShareIDs.length" @click="batch(true)">重新识别失败项</n-button>
        <n-button type="success" tertiary :disabled="!selectedShareIDs.length" @click="batch(false, true)">继续识别待识别内容</n-button>
        <n-button type="info" :disabled="!selectedShareIDs.length" @click="syncSelected">批量同步分享文件</n-button>
        <n-button type="primary" secondary :disabled="!selectedShareIDs.length" @click="processSelected">一键处理全部</n-button>
        <n-button type="warning" @click="taskSettingsShow=true">识别任务设置</n-button>
        <n-button type="error" :loading="clearingAll" :disabled="clearingAll || !selectedShareIDs.length" @click="clearSelectedMedia">批量清空识别内容</n-button>
      </n-space>
      <n-alert v-if="taskState" class="mb-4" :type="taskState.status === 'failed' ? 'error' : taskState.status === 'completed' ? 'success' : 'info'" :title="`分享任务：${taskPhase}`">
...
const clearSelectedMedia = () => {
  ...
  dialog.warning({
    title: '批量清空识别内容',
    content: `确定清空选中的 ${ids.length} 个分享下的全部媒体记录吗？分享链接及配置会保留，网盘文件不受影响。`,
...
const restoreTask=async()=>{try{const r=await getUnifiedTaskList();const payload=r.data?.data||r.data;const tasks=Array.isArray(payload)?payload:(payload?.data||[]);const active=tasks.filter(t=>['share_identify','share_sync'].includes(t.task_type)&&['pending','running'].includes(t.status)).sort((a,b)=>String(b.update_time||'').localeCompare(String(a.update_time||'')))[0];if(active?.task_id)void watchTask(active.task_id)}catch(e){/* 任务恢复失败不阻塞分享列表 */}}
const openBatchImport = () => { batchImportShow.value = true }
const openCreate=()=>{editing.value=false;form.value={name:'',url:'',password:'',note:'',media:[]};show.value=true};const openEdit=r=>{editing.value=true;form.value={id:r.id,version:r.version,name:r.name,url:r.url,password:r.password,note:r.note,media_type:r.media_type||'auto'};show.value=true};const save=async()=>{if(editing.value)await updateShareRecord(form.value.id,form.value);else await createShareRecord(form.value);show.value=false;load()};const pollTask=async id=>{try{const r=await getShareIdentifyTask(id);taskState.value=r.data?.data||r.data;if(['completed','failed','cancelled'].includes(taskState.value?.status)){clearInterval(taskTimer.value);taskTimer.value=null;load();return taskState.value.status}}catch(e){clearInterval(taskTimer.value);taskTimer.value=null;throw e}};const watchTask=async id=>{taskState.value={task_id:id,status:'pending',progress:0,metadata:{phase:'准备中',steps:[]}};const terminal=await pollTask(id);if(terminal)return terminal;return new Promise((resolve,reject)=>{taskTimer.value=setInterval(async()=>{try{const status=await pollTask(id);if(status)resolve(status)}catch(e){reject(e)}},1000)})};const batch=async (retry,pendingOnly=false,shareIDs=selectedShareIDs.value)=>{const r=await batchIdentifyShareRecords({share_ids:shareIDs,retry_failed:retry,pending_only:pendingOnly});const id=r.data?.data?.task_id;if(id)await watchTask(id);else msg.error('批量识别任务创建失败')};const syncSelected=async shareIDs=>{const response=await batchSyncShareRecords(shareIDs||selectedShareIDs.value);const id=response.data?.data?.task_id;if(id)return watchTask(id);msg.error('批量同步分享文件任务创建失败')};const processSelected=async()=>{const shareIDs=[...selectedShareIDs.value];await syncSelected(shareIDs);const syncedIDs=taskState.value?.metadata?.synced_share_ids||[];if(syncedIDs.length)await batch(false,false,syncedIDs);else msg.warning('没有同步成功的分享，未发起识别任务')};const syncRecord=async r=>{const response=await syncShareRecord(r.id);const id=response.data?.data?.task_id;if(id)await watchTask(id);else msg.error('分享文件同步任务创建失败')};const identifyRecord=async (r,pendingOnly=false,failedOnly=false)=>{const response=await identifyShareRecord(r.id,pendingOnly,failedOnly);const id=response.data?.data?.task_id;if(id)await watchTask(id);else msg.error('分享识别任务创建失败')};const identify=async m=>{await identifyShareMedia(m.id,{file_name:m.file_name,metadata_source:m.metadata_source,version:m.version});load()};const importFile=async({file})=>{for(const r of JSON.parse(await file.file.text()))await createShareRecord(r);load()};const remove=async r=>{await deleteShareRecord(r.id);load()};onBeforeUnmount(()=>{if(taskTimer.value)clearInterval(taskTimer.value)})
