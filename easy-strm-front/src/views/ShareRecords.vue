...
import { getShareRecords, clearShareMedia, createShareRecord, updateShareRecord, deleteShareRecord, identifyShareMedia, identifyShareRecord, deleteShareMedia, batchIdentifyShareRecords, getShareIdentifyTask } from '../utils/api/media'
...
const restoreTask=async()=>{try{const r=await getUnifiedTaskList();const payload=r.data?.data||r.data;const tasks=Array.isArray(payload)?payload:(payload?.data||[]);const active=tasks.filter(t=>t.task_type==='share_identify'&&['pending','running'].includes(t.status)).sort((a,b)=>String(b.update_time||'').localeCompare(String(a.update_time||'')))[0];if(active?.task_id)await watchTask(active.task_id)}catch(e){/* 任务恢复失败不阻塞分享列表 */}}
const openBatchImport = () => { batchImportShow.value = true }
const openCreate=()=>{editing.value=false;form.value={name:'',url:'',password:'',note:'',media:[]};show.value=true};const openEdit=r=>{editing.value=true;form.value={id:r.id,version:r.version,name:r.name,url:r.url,password:r.password,note:r.note,media_type:r.media_type||'auto'};show.value=true};const save=async()=>{if(editing.value)await updateShareRecord(form.value.id,form.value);else await createShareRecord(form.value);show.value=false;load()};const pollTask=async id=>{try{const r=await getShareIdentifyTask(id);taskState.value=r.data?.data||r.data;if(['completed','failed','cancelled'].includes(taskState.value?.status)){clearInterval(taskTimer.value);taskTimer.value=null;load()}}catch(e){clearInterval(taskTimer.value);taskTimer.value=null}};const watchTask=async id=>{taskState.value={task_id:id,status:'pending',progress:0,metadata:{phase:'准备中',steps:[]}};await pollTask(id);taskTimer.value=setInterval(()=>pollTask(id),1000)};const batch=async (retry,pendingOnly=false)=>{const r=await batchIdentifyShareRecords({retry_failed:retry,pending_only:pendingOnly});const id=r.data?.data?.task_id;if(id)await watchTask(id);else msg.error('批量识别任务创建失败')};const identifyRecord=async (r,pendingOnly=false,failedOnly=false)=>{const response=await identifyShareRecord(r.id,pendingOnly,failedOnly);const id=response.data?.data?.task_id;if(id)await watchTask(id);else msg.error('分享识别任务创建失败')};const identify=async m=>{await identifyShareMedia(m.id,{file_name:m.file_name,metadata_source:m.metadata_source,version:m.version});load()};const importFile=async({file})=>{for(const r of JSON.parse(await file.file.text()))await createShareRecord(r);load()};const remove=async r=>{await deleteShareRecord(r.id);load()};onBeforeUnmount(()=>{if(taskTimer.value)clearInterval(taskTimer.value)})

const columns=[{type:'expand',expandable:r=>(r.identified_count||0)>0,renderExpand:r=>h(ShareMediaGallery,{shareId:r.id,revision:galleryRevision.value,onIdentify:identify,onSaved:load,onRemove:async m=>{await deleteShareMedia(r.id,m.id);await load()}})},{title:'名称',key:'name',render:r=>h(NSpace,{align:'center'},()=>[h('span',r.name),r.share_cancelled?h(NTag,{type:'error',size:'small'},()=> '分享已取消'):null])},{title:'链接',key:'url',ellipsis:true},{title:'媒体数',render:r=>h('div',{},[h('div',{},'总数 '+(r.media_count||0)),h('div',{class:'text-xs text-slate-500'},'已识别 '+(r.identified_count||0)+' · 失败 '+(r.failed_count||0)+' · 待识别 '+(r.pending_count||0)+' · 脱敏 '+(r.masked_count||0))])},{title:'操作',render:r=>h('div',{class:'flex flex-wrap gap-2'},[h(NButton,{size:'small',type:'warning',loading:clearingIDs.value.has(r.id),disabled:clearingIDs.value.has(r.id),onClick:()=>clearMedia(r)},{default:()=>'清空识别内容'}),h(NButton,{size:'small',onClick:()=>openEdit(r)},{default:()=>'编辑'}),h(NButton,{size:'small',onClick:()=>remove(r)},{default:()=>'删除'}),h(NButton,{size:'small',type:'primary',onClick:()=>identifyRecord(r)},{default:()=>'识别'}),h(NButton,{size:'small',type:'primary',secondary:true,disabled:r.share_cancelled||!r.pending_count,onClick:()=>identifyRecord(r,true)},{default:()=>'继续识别待识别'}),h(NButton,{size:'small',type:'warning',secondary:true,disabled:r.share_cancelled||!r.failed_count,onClick:()=>identifyRecord(r,false,true)},{default:()=>'识别失败内容'})])}]
