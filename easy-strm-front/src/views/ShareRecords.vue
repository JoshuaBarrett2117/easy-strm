...
<script setup>
import { h, ref, onBeforeUnmount, onMounted, watch } from 'vue'
...
const restoreTask = async () => {
  try {
    ...
    for (const task of tasks.filter(t => ['share_identify', 'share_sync'].includes(t.task_type) && ['pending', 'running'].includes(t.status))) {
      void watchTask(task.task_id).catch(() => msg.error('分享任务进度获取失败，请刷新重试'))
    }
  } catch { /* 任务恢复失败不阻塞分享列表 */ }
}
const openBatchImport = () => { batchImportShow.value = true }
const openCreate=()=>{editing.value=false;form.value={name:'',url:'',password:'',note:'',media:[]};show.value=true};const openEdit=r=>{editing.value=true;form.value={id:r.id,version:r.version,name:r.name,url:r.url,password:r.password,note:r.note,media_type:r.media_type||'auto'};show.value=true};const save=async()=>{if(editing.value)await updateShareRecord(form.value.id,form.value);else await createShareRecord(form.value);show.value=false;load()};const batch=async (retry,pendingOnly=false,shareIDs=selectedShareIDs.value)=>{const r=await batchIdentifyShareRecords({share_ids:shareIDs,retry_failed:retry,pending_only:pendingOnly});const id=r.data?.data?.task_id;if(id)await watchTask(id);else msg.error('批量识别任务创建失败')};const syncSelected=async shareIDs=>{const response=await batchSyncShareRecords(shareIDs||selectedShareIDs.value);const id=response.data?.data?.task_id;if(id)return watchTask(id);msg.error('批量同步分享文件任务创建失败')};const loadAllShareIDs=async()=>{const ids=[];let pageNumber=1;let total=0;do{const response=await getShareRecords({page:pageNumber,page_size:200});const payload=response.data?.data||response.data||{};const records=Array.isArray(payload)?payload:(payload.data||[]);ids.push(...records.map(record=>record.id).filter(id=>Number.isInteger(id)));total=Number(payload.total||records.length);pageNumber++;if(!records.length)break}while(ids.length<total);return [...new Set(ids)]};const processSelected=async()=>{const shareIDs=await loadAllShareIDs();if(!shareIDs.length){msg.warning('没有可处理的分享');return}const syncTask=await syncSelected(shareIDs);if(!syncTask)return;const syncedIDs=syncTask.metadata?.synced_share_ids||[];if(syncedIDs.length)await batch(false,false,syncedIDs);else msg.warning('没有同步成功的分享，未发起识别任务')};const syncRecord=async r=>{const response=await syncShareRecord(r.id);const id=response.data?.data?.task_id;if(id)await watchTask(id);else msg.error('分享文件同步任务创建失败')};const identifyRecord=async (r,pendingOnly=false,failedOnly=false)=>{const response=await identifyShareRecord(r.id,pendingOnly,failedOnly);const id=response.data?.data?.task_id;if(id)await watchTask(id);else msg.error('分享识别任务创建失败')};const identify=async m=>{await identifyShareMedia(m.id,{file_name:m.file_name,metadata_source:m.metadata_source,version:m.version});load()};const importFile=async({file})=>{for(const r of JSON.parse(await file.file.text()))await createShareRecord(r);load()};const remove=async r=>{await deleteShareRecord(r.id);load()};onBeforeUnmount(()=>taskWatcher.dispose())
