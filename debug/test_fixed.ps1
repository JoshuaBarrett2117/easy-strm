# Simple API Test Script - Fixed Version
...
try {
    ...
    if (-not $sourceId) {
        Write-Host "  [WARN] Source ID is empty, trying to get from list" -ForegroundColor Yellow
        $listResp = Invoke-RestMethod -Uri "http://localhost:8082/media/sources" -Method GET -Headers $headers
        $source = $listResp.data | Where-Object { $_.name -eq "TestUpdateSource" } | Select-Object -First 1
        if ($source) {
            $sourceId = $source.id
            Write-Host "  Found source ID from list: $sourceId" -ForegroundColor Green
        }
    }
} catch {
...
if (-not $sourceId) {
    Write-Host "Cannot continue without source ID" -ForegroundColor Red
    exit 1
}
