...
func TestShareCandidateIdempotency(t *testing.T) {
	for _, exists := range []bool{true, false} {
		...
		q := mock.ExpectQuery("SELECT f.id,f.version FROM t_share_media_file").WithArgs(9, "剧集")
		if exists {
			q.WillReturnRows(sqlmock.NewRows([]string{"id", "version"}).AddRow(7, 2))
		} else {
			q.WillReturnError(sql.ErrNoRows)
			mock.ExpectQuery("INSERT INTO t_share_media_file").WithArgs(9, "剧集", "auto").WillReturnRows(sqlmock.NewRows([]string{"id", "version"}).AddRow(7, 1))
		}
		mock.ExpectCommit()
