...
// EnsureMediaLibraryTables 确保媒体库管理底座表存在。
func EnsureMediaLibraryTables() error {
	if DB == nil {
		return fmt.Errorf("EnsureMediaLibraryTables 数据库未初始化")
	}
	...
	statements := []string{
		...
		`CREATE UNIQUE INDEX IF NOT EXISTS idx_media_sync_index_source_file ON t_media_sync_index(source_id, source_file_id)`,
		`CREATE INDEX IF NOT EXISTS idx_media_sync_index_source ON t_media_sync_index(source_id, sync_status)`,
		`CREATE INDEX IF NOT EXISTS idx_media_sync_index_tmdb ON t_media_sync_index(tmdb_id, media_type)`,
		...
		`CREATE UNIQUE INDEX IF NOT EXISTS idx_task_step_task_step ON t_task_step(task_id, step_key)`,
		`CREATE INDEX IF NOT EXISTS idx_task_step_task ON t_task_step(task_id, sort_order)`,
		...
		`CREATE INDEX IF NOT EXISTS idx_pending_media_status ON t_pending_media_item(status, updated_at DESC)`,
		`CREATE INDEX IF NOT EXISTS idx_pending_media_source ON t_pending_media_item(source_id, source_file_id)`,
	...
	for _, statement := range statements {
		if _, err := DB.Exec(statement); err != nil {
			return fmt.Errorf("EnsureMediaLibraryTables 执行失败: %v", err)
		}
	}
	return nil
}
